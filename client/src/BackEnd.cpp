...
QString BackEnd::getHistory() const
{
  return History;
}
...
void BackEnd::setPoint(const QString &value)
{
  point = value;
}
...
void BackEnd::signIn(QString username, QString password)
{
  ...
  Message loginStatus = login(user, pass);
  printf("%s\n", loginStatus.value);
  //printf("%s\n", login(user, pass).value);
  if (loginStatus.type == LOGIN_SUCCESS)
  {
    user_name = username;
    strcpy(username1, user);
    char Rank[20];
    char PointP[20];
    sscanf(loginStatus.value,"%s %s", Rank, PointP);
    Point = atoi(PointP);
    rank = Rank;
    point = PointP;
    emit loginSuccess();
  }
  else if (loginStatus.type == LOGGED_IN)
  {
    emit loggedIn();
  }
  else if (loginStatus.type == ACCOUNT_BLOCKED)
  {
    emit accountBlocked();
  }
  else if (loginStatus.type == ACCOUNT_NOT_EXIST)
  {
    emit accountNotExist();
  }
  else if (loginStatus.type == WRONG_PASSWORD)
  {
    emit wrongPassword();
  }
}

void BackEnd::storeHistory()
{
  char* resultString[1024*10];
  //resultString[0]='\0';
  char username[1024] ;
  strcpy(username, username1);
  char filename[1024];
  char HistoryUser[1024];
  sprintf(filename, "/home/vboxuser/Downloads/AiLaTrieuPhu-main/server/logfile/%s.txt", username);
  strcpy(HistoryUser,getUserHistory(resultString,sizeof(resultString),filename));
  printf("%s",HistoryUser);
  History = HistoryUser;
  emit HistoryChanged();
}

void BackEnd::changedPoint()
{
  if(BackEnd::instance->status_game == WIN)
  {
    Point = Point + 100;
    char str[20];
    sprintf(str,"%d",Point);
